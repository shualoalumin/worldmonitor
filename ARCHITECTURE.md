...
| `daily-brief.yml` | Weekday 06:00 UTC cron, push to main (brief-script paths), manual | Generates the daily intelligence brief via `scripts/daily-brief.mjs` and retains it as an artifact; the push trigger runs the script's offline test suite only (generation is skipped without the `WORLDMONITOR_API_KEY` secret) |
